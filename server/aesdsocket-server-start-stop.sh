...
case "$1" in
    start)
        echo "Starting aesdsocket server as a daemon process...."
        start-stop-daemon -S -n aesdsocket_server -a /usr/bin/aesdsocket_server -- -d
        ;;
    
    stop)
         echo "Stoping aesdsocket server as a daemon process...."
